...
| _timeout_ | \<time\> | 60&nbsp;seconds | the time since the stream started after which we consider this a stalled stream and give up on it. If specified, this overrides the default _timeout_ attribute of the **start** action for this stream. If this is set to 0 and is not set in **start** action, then an internally defined timeout is used instead (currently 60 seconds). |
| _stallout_ | \<time\> | 15&nbsp;seconds | the time since bytes were last sent/received for this stream after which we consider this a stalled stream and give up on it. If specified, this overrides the default _stallout_ attribute of the **start** element for this stream. If this is set to 0 and is not set in **start** action, then an internally defined stallout is used instead (currently 15 seconds). |
